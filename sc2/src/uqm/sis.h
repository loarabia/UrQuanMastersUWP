...
typedef struct
{
	...
	UQM_UTF8_T ShipName[SIS_NAME_SIZE];
	UQM_UTF8_T CommanderName[SIS_NAME_SIZE];
	UQM_UTF8_T PlanetName[SIS_NAME_SIZE];
} SIS_STATE;
...
extern void DrawSISTitle (UQM_UTF8_T *pStr);
...
extern BOOLEAN DrawSISMessageEx (const UQM_UTF8_T *pStr, SIZE CurPos,
		SIZE ExPos, COUNT flags);

extern void DrawSISMessage (const UQM_UTF8_T *pStr);
...
extern void DrawStatusMessage (const UQM_UTF8_T *pStr);
typedef enum
{
	SMM_UNDEFINED = 0,
	SMM_DATE,
